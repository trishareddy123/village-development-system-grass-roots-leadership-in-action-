...
{
	echo "<script>window.location='volunteeraccount.php';</script>";
}
$err='';
if(isset($_POST['submit']))
{
	$sql = "SELECT * FROM volunteer WHERE loginid='$_POST[loginid]' AND password='$_POST[password]' AND status='Active'";
	$qsql = mysqli_query($con,$sql);
	if(mysqli_num_rows($qsql) == 1)
	{
		$rslogin = mysqli_fetch_array($qsql);
		$_SESSION['volunteerid']= $rslogin['volunteerid'] ;
		echo "<script>window.location='volunteeraccount.php';</script>";
	}
	else
	{
...
?></div>
    ...
    <div class="card">
        <h1 class="title"><span>Grassroots Leadership in Action</span>Login <span class="msg">Hello, Volunteer!</span></h1>
        <div class="col-md-12">
...
{
	 if(document.frmvollogin.loginid.value == "")
	{
		alert("Login ID should not be empty..");
		document.frmvollogin.loginid.focus();
		return false;
	}
	else if(!document.frmvollogin.loginid.value.match(alphanumericExp))
	{
		alert("Login ID not valid..");
		document.frmvollogin.loginid.focus();
		return false;
	}
	else if(document.frmvollogin.password.value == "")
	{
		alert("Password should not be empty..");
		document.frmvollogin.password.focus();
		return false;
	}
	else if(document.frmvollogin.password.value.length < 8)
	{
